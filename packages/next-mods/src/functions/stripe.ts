...
export const stripe: FunctionModule = {
  name: "stripe",
  description:
    "Add Stripe payment processing effortlessly to your Next.js project.",
  uninstall: async () => {
    try {
  ...
  install: async (options: Record<string, any> = {}) => {
    try {
      ...
      const projectRoot = process.cwd();
      // console.log(projectRoot, "PROJECT ROOT");
      ...
      if (!options.includes("repair")) {
        ...
        logger.info(
          "> Please goto https://dashboard.stripe.com and obtain your secret key."
        );
        const STRIPE_SECRET_KEY = await input({
          message: "Enter your Stripe Secret Key:",
          validate: (value) =>
            value.startsWith("sk_test")
              ? true
              : "Please enter a valid secret key starting with sk_'",
        });
        addEnvVariable("STRIPE_SECRET_KEY", STRIPE_SECRET_KEY);
        logger.info(
          "> Please goto https://dashboard.stripe.com/test/webhooks and create a new webhook. You can follow the instructions here: https://stripe.com/docs/webhooks/setup."
        );
        const STRIPE_WEBHOOK_SECRET = await input({
          message: "Enter your webhook key:",
          validate: (value) => {
            if (value.length >= 200) {
              return true;
            } else {
        ...
        addEnvVariable("STRIPE_WEBHOOK_SECRET", STRIPE_WEBHOOK_SECRET);
      }

      // Determine the installation command
      let installCommand;
      if (packageManager === "yarn") {
        installCommand = "yarn add stripe";
      } else if (packageManager === "pnpm") {
        installCommand = "pnpm add stripe";
      } else {
        installCommand = "npm install stripe";
      }

      // Install Supabase package
      logger.info(`> Installing stripe using ${packageManager}...`);
      try {
        execSync(installCommand, { stdio: "ignore" });
        logger.success(`> Stripe package installed successfully.`);
      } catch (error) {
        logger.error(
          `> Error installing Stripe package:`,
      ...
      const stripeDir = path.join(projectRoot, "lib", "stripe");

      try {
        const supabaseClientPath = path.join(stripeDir, "supabaseClient.ts");
        fs.ensureDirSync(stripeDir);
...
`;
        ...
        const supabaseServerPath = path.join(stripeDir, "supabaseServer.ts");
...
`;
      ...
      if (createMiddleware) {
        ...
        try {
          const supabaseMiddlewarePath = path.join(stripeDir, "middleware.ts");
...
export const config = {
  matcher: [
    ...
    '/((?!_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)',
      ...
      const rootMiddlewarePath = path.join(stripeDir, "supabaseActions.ts");
      logger.info(
